...
public class PlayerController implements Initializable {
    ...
    private void handleReady() {
        ...
        lengthText.setText(String.format("%.1f", durationInMillis));
        Duration ct = mediaPlayer.getCurrentTime();
        Double currentInMillis = ct.toMillis();
        currentText.setText(String.format("%.1f", currentInMillis));
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    @FXML
    private void handleReleased(Event event) {
        if (mediaPlayer != null) {
